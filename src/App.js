...
const LinksAndDM = () => {
  ...
  useEffect(() => {
    const savedProfile = localStorage.getItem('linksAndDmProfile');
    if (savedProfile) setProfile(JSON.parse(savedProfile));

    const savedDmButtons = localStorage.getItem('linksAndDmButtons');
    if (savedDmButtons) setDmButtons(JSON.parse(savedDmButtons));

    const savedCharityLinks = localStorage.getItem('linksAndDmCharityLinks');
    if (savedCharityLinks) setCharityLinks(JSON.parse(savedCharityLinks));

    const savedCategoryButtons = localStorage.getItem('linksAndDmCategoryButtons');
    if (savedCategoryButtons) setCategory(JSON.parse(savedCategoryButtons));

    const savedPortfolio = localStorage.getItem('linksAndDmPortfolio');
    if (savedPortfolio) setPortfolio(JSON.parse(savedPortfolio));

    const savedProjects = localStorage.getItem('linksAndDmProjects');
    if (savedProjects) setProjects(JSON.parse(savedProjects));

    const savedPriorityContacts = localStorage.getItem('linksAndDmPriorityContacts');
    if (savedPriorityContacts) setPriorityContacts(JSON.parse(savedPriorityContacts));
  }, []);

  // Save profile to localStorage
  useEffect(() => {
    localStorage.setItem('linksAndDmProfile', JSON.stringify(profile));
  }, [profile]);

  useEffect(() => {
    localStorage.setItem('linksAndDmButtons', JSON.stringify(dmButtons));
  }, [dmButtons]);

  useEffect(() => {
    localStorage.setItem('linksAndDmCharityLinks', JSON.stringify(charityLinks));
  }, [charityLinks]);

  useEffect(() => {
    localStorage.setItem('linksAndDmCategoryButtons', JSON.stringify(categoryButtons));
  }, [categoryButtons]);

  useEffect(() => {
    localStorage.setItem('linksAndDmPortfolio', JSON.stringify(portfolio));
  }, [portfolio]);

  useEffect(() => {
    localStorage.setItem('linksAndDmProjects', JSON.stringify(projects));
  }, [projects]);

  useEffect(() => {
    localStorage.setItem('linksAndDmPriorityContacts', JSON.stringify(priorityContacts));
  }, [priorityContacts]);
  ...
  // Handle message form submission with Firebase
  ...
  // Toggle star on message with Firebase update
              ...
              sortedMessages.map((msg) => (
                <div key={msg.id} className="bg-white rounded-2xl p-4 shadow-lg border-4 border-purple-200 hover:shadow-xl transition">
                  ...
                  <p className="text-xs text-gray-400">
                    {msg.timestamp && new Date(msg.timestamp.toDate ? msg.timestamp.toDate() : msg.timestamp).toLocaleString()}
                  </p>
                </div>
              ))
            )}
            ...
            <div className="space-y-4">
              {Object.entries(dmButtons).map(([key, btn]) => (
                <div key={key} className="bg-white/95 rounded-2xl p-5 border-2 border-white/50">
                  ...
                  {key === 'supportCause' && (
                    <>
                      <label className="block font-bold text-sm mb-1 text-gray-600">Note: Add charities below</label>
                    </>
                  )}
                </div>
              ))}
